...
func New(filePath string) (*Logger, error) {
	// Ensure the directory exists
	dir := strings.TrimSuffix(filePath, "/"+strings.TrimPrefix(filePath, "/"))
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		err = os.MkdirAll(dir, 0700) // Create directory with secure permissions
		if err != nil {
			return nil, fmt.Errorf("failed to create log directory: %w", err)
		}
	}

	// Open the log file with append mode and create if not exists
...
func (l *Logger) log(level LogLevel, message string) {
	...
	sanitizedMsg := sanitizeInput(message)
	logLine := fmt.Sprintf("[%s] [%s] %s\n", timestamp, level, sanitizedMsg)
	...
	// Write errors to stderr for better visibility
	if level == ERROR {
		fmt.Fprintf(os.Stderr, logLine)
	}
}

// sanitizeInput removes any potentially harmful characters from log messages
func sanitizeInput(input string) string {
